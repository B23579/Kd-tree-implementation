...
result<-read.csv("hybride.txt", header = FALSE,sep=";") 
result%>%rename(Thread_number=V1, n=V2, walltime=V3)-> result
result$Thread_number<- as.integer(result$Thread_number)
result$n <- as.integer(result$n)
result$walltime <- as.numeric(result$walltime) 
...
p<-ggplot(data=re)+
  ...
  theme(plot.title = element_text(hjust = 0.3))
p
ggsave(p, filename = "strongscalabilityhy.png")
...
ggsave(p, filename = "weakscalinghy.png")
...
resu<-filter(result, result$Thread_number==1)
resu1<-filter(result, result$Thread_number==4)
resu2<-filter(result, result$Thread_number==8)
resu3<-filter(result, result$Thread_number==16)
...
total <- rbind(resu,resu1,resu2,resu3)
...
p
ggsave(p,filename = "efficientyhy.png")
...
total$n<-as.integer(as.character(total$n))
weak<-filter(total,total$Thread_number==4,n==6)%>%
  rbind(filter(total,total$Thread_number==8,total$n==7))%>%
  rbind(filter(total,total$Thread_number==16,total$n==8))
...
p 
ggsave(p,filename = "weakscalabilityHy.png")
...
p 
ggsave(p,filename = "weakspeedupHy.png")
...
p 
ggsave(p,filename = "weakEfficiencyhyd.png")
